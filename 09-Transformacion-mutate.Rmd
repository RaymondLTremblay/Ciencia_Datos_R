...
df3=df %>% 
  dplyr::select(value) %>% 
  mutate(Prom_cum=cummean(value)) %>% # dplyr:: es para especificar que la función cummean es del paquete dplyr
  mutate(Var_cum=TidyDensity::cvar(value)) |> 
  mutate(promedio_coriendo= zoo::rollmean(value, k=7, fill=NA, align="left")) |> 
  mutate(varianza_coriendo= zoo::rollapply(value, width=7, FUN=var, fill=NA, align="left"))
